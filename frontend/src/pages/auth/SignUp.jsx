...
export default function SignUp() {
    ...
    try {
      ...
      if (err.response?.data) {
        ...
        if (!hasFieldError) {
          ...
          setNotice({ type: "error", message: genericMsg });
        }
      } else {
        setNotice({
          type: "error",
  ...
  const handleResend = async () => {
    if (!pendingVerification.user_id && !pendingVerification.email) {
      setNotice({
        type: "error",
        message:
          "No account details are available to resend the verification email.",
      });
      return;
    }
    ...
    try {
      await auth.requestVerification({
        user_id: pendingVerification.user_id || undefined,
        ...
        <div className="relative w-full lg:w-[52%] overflow-hidden min-h-[300px]">
          ...
          <div className="absolute bottom-6 left-6 right-6 flex gap-3">
            {[
              ...
              >
                <p
                  className={`text-lg font-semibold ${s.accent ? "text-yellow-400" : "text-white"}`}
                >
                  {s.value}
                </p>
        ...
        <div className="w-full lg:w-[48%] bg-white flex items-center justify-center px-8 py-12 lg:px-14 overflow-y-auto max-h-screen">
          <div className="w-full max-w-sm">
            ...
            <p className="text-sm text-gray-400 mt-1 mb-8">
              Join LibraryHub today
            </p>

            {/* Success message */}
            {showSuccess && (
              <div className="bg-green-50 border border-green-200 text-green-600 rounded-2xl px-4 py-3 mb-5 text-sm">
            ...
            {/* Notice message for success/error feedback */}
            {notice.message && (
              <div
                className={`rounded-2xl px-4 py-3 mb-5 text-sm flex items-center gap-2 ${
                  notice.type === "success"
