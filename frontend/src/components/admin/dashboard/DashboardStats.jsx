...
import { SkeletonText, SkeletonCard } from '../../common/Skeleton';

const DashboardStats = ({ data, isLoading }) => {
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
      {isLoading ? (
        /* Render 4 dummy cards if loading */
        [1, 2, 3, 4].map(key => (
          <div key={key} className="bg-[#fcfaf8] rounded-2xl p-6 shadow-sm border border-orange-100/50 flex flex-col justify-between min-h-[160px]">
             <div className="flex justify-between items-start">
               <div className="space-y-2">
                 <SkeletonText className="h-8 w-16 rounded" />
                 <SkeletonText className="h-4 w-24 rounded" />
               </div>
               <SkeletonCard className="w-20 h-10 rounded" />
             </div>
             <div className="pt-4 mt-6 border-t border-orange-100/50 flex justify-between items-center">
               <SkeletonText className="h-3 w-20 rounded" />
               <SkeletonText className="h-3 w-24 rounded" />
             </div>
          </div>
        ))
      ) : (
